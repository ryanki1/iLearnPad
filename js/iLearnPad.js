...
var CartCollectionView = Backbone.View.extend({
  ...
  renderMenuItem: function(item) {
    var itemMenuView = new ItemMenuView({ model: item, tagName: 'li', attributes: {"class":"ui-btn ui-btn-icon-right ui-li-has-arrow ui-li ui-corner-top ui-corner-bottom ui-btn-up-c"} });
  ...
  renderItem: function(item) {
    ...
    $( "#content" + item.attributes.title ).bind( 'pageinit',function(event){
    	//debugger;    	
    	//Preload audio for page
    	if (!item.attributes.audio){
    		var audio = [];
	    	for (var i=0; i<item.attributes.soundBites.length;i++){
	    		audio[i] = new Audio(item.attributes.pathSound + item.attributes.soundBites[i] + item.attributes.soundType)
	    	}
	    	item.attributes.audio = audio;	
    	}
	    $('[id^="area'+item.attributes.title+'"]').bind("tap", function () {
	    	//debugger;
	        twangIt(this, item);
		});
		$( "#content" + item.attributes.title ).bind( 'pagebeforeshow',function(event){
	    ...
	    $( "#content" + item.attributes.title ).bind( 'pageshow',function(event){
	  		...
	  		$('#img' + item.attributes.title).attr('src', (window.orientation == 90 || window.orientation == -90) ? item.attributes.pathPortrait : item.attributes.pathPortrait);
	  		//alert( 'Orientation: ' +  window.orientation + "window.orientation==90|-90:" + (window.orientation == 90 || window.orientation == -90));
	  		//alert('img src: ' + (window.orientation == 90 || window.orientation == -90) ? item.attributes.pathPortrait : item.attributes.pathPortrait);
			$('#img' + item.attributes.title).attr('usemap', (window.orientation == 90 || window.orientation == -90) ? "#"+item.attributes.usemapPortrait : "#"+item.attributes.usemapPortrait);
			//alert( 'Orientation: ' +  window.orientation + "window.orientation==90:" + (window.orientation==90));
			//alert('img usemap: ' + (window.orientation == 90 || window.orientation == -90) ? "#"+item.attributes.usemapPortrait : "#"+item.attributes.usemapPortrait);
...
function twangIt(area, item) {        
        ...
        var soundBite = item.attributes.audio[item.attributes.soundBites.indexOf($(area).attr('id'))];
        if (soundBite) {
            soundBite.play();
        }        
    }

// Splash page - thanks to http://stackoverflow.com/questions/6332989/how-to-add-full-screen-welcome-image-on-a-web-app-using-jquerymobile
/*$(function() {
...
/*window.onbeforeonload = function(){
	alert("window.onbeforeonload called");
	$.mobile.changePage($("[id='splash']"),{allowSamePageTransition:true});
	setTimeout(function(){var x;}, 10000);
}*/
...
$(function() {
  ...
  //$.mobile.changePage($("[id='splash']"),{allowSamePageTransition:true});	
  //setTimeout(function(){$.mobile.changePage($("[id='menu']"),{allowSamePageTransition:true});}, 2000);
  //fnA(); 
});	